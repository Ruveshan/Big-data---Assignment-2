...
# Calculate the median price for the stock indices sector during COVID and post-COVID
median_during_covid <- during_covid_data %>%
  summarise(across(all_of(indices_prices), median, na.rm = TRUE)) %>%
  rowMeans(na.rm = TRUE)
...
print("Median Prices of Stock Indices Sector During and Post-COVID:")
print(median_indices)

# Plot the median prices comparison
ggplot(median_indices, aes(x = Period, y = Median_Price, fill = Period)) +
  geom_bar(stat = "identity", width = 0.5) +
  labs(
    title = "Median Price of Stock Indices Sector During and Post-COVID",
    x = "Period",
    y = "Median Price (USD)"
  ) +
