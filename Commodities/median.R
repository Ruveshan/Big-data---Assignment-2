...
# Calculate the median price for the commodities sector during COVID and post-COVID
median_during_covid <- during_covid_data %>%
  summarise(across(all_of(commodities_prices), median, na.rm = TRUE)) %>%
  rowMeans(na.rm = TRUE)
...
print("Median Prices of Commodities Sector During and Post-COVID:")
print(median_commodities)

# Plot the median prices comparison
ggplot(median_commodities, aes(x = Period, y = Median_Price, fill = Period)) +
  geom_bar(stat = "identity", width = 0.5) +
  labs(
    title = "Median Price of Commodities Sector During and Post-COVID",
    x = "Period",
    y = "Median Price (USD)"
  ) +
