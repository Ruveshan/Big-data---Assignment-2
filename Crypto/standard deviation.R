...
# Calculate the standard deviation for the cryptocurrency sector during COVID and post-COVID
std_dev_during_covid <- during_covid_data %>%
  summarise(across(all_of(crypto_prices), sd, na.rm = TRUE)) %>%
  rowMeans(na.rm = TRUE)
...
print("Std. Deviation of Crypto Sector Prices During and Post-COVID:")
print(std_dev_crypto)

# Plot the standard deviation comparison
ggplot(std_dev_crypto, aes(x = Period, y = Standard_Deviation, fill = Period)) +
  geom_bar(stat = "identity", width = 0.5) +
  labs(
    title = "Std. Deviation of Crypto Sector Prices During and Post-COVID",
    x = "Period",
