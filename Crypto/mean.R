...
# Calculate the mean price for the cryptocurrency sector during COVID and post-COVID
mean_during_covid <- during_covid_data %>%
  summarise(across(all_of(crypto_prices), mean, na.rm = TRUE)) %>%
  rowMeans(na.rm = TRUE)
...
print("Mean Prices of Cryptocurrency Sector During and Post-COVID:")
print(mean_crypto)

# Plot the mean prices comparison
ggplot(mean_crypto, aes(x = Period, y = Mean_Price, fill = Period)) +
  geom_bar(stat = "identity", width = 0.5) +
  labs(
    title = "Mean Price of Cryptocurrency Sector During and Post-COVID",
    x = "Period",
    y = "Mean Price (USD)"
  ) +
